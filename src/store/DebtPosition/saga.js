...
import { setDebtPosition, setFetching, setFinancialCards } from 'store/DebtPosition/action';
import { GET_DEBT_POSITION, SUBMIT_FEEDBACK } from 'store/DebtPosition/constants';
...
export function* getDebtPositionSaga(action) {
  ...
  try {
    const response = yield all([
      SagaLib.mutationCall(GET_DEBT_POSITION_QUERY, {
        query: {
          dataSets: [
            {
              type: 'net-worth',
    ...
    // yield delay(2000);
    yield put(setDebtPosition(values));
...
export default function* defaultSaga() {
  yield takeEvery(GET_DEBT_POSITION, getDebtPositionSaga);
