...
import { initialState } from 'store/CashPosition/reducer';

export const makeSelectDomain = state => state.cashPosition || initialState;
...
export const selectCashPosition = createSelector(makeSelectDomain, state => {
...
export const selectChartData = createSelector(makeSelectDomain, state => {
  // Extract chartTime and clone cashPosition array to avoid modifying the original state
  const chartTime = state.chartTime;
  return getLineChartGraph(chartTime, state.cashPosition);
});

export const selectChartDataHomeScreen = createSelector(makeSelectDomain, state => {
  return getLineChartGraph('Y', state.cashPosition);
});
