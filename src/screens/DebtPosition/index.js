...
import { getDebtPosition, setChartTime } from 'store/DebtPosition/action';
import getModule from 'store/DebtPosition/module';
import {
...
} from 'store/DebtPosition/selector';
import { AppStyle } from 'theme';

import themedStyles from './styles';

const i18nScope = 'screens.debtPosition';

function DebtPositionScreen(props) {
  ...
  const chartData = useSelector(selectDebtPosition);
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    dispatchResolve(getDebtPosition(true)).then(() => {
      setRefreshing(false);
    });
  }, [dispatchResolve]);
      ...
      <ScrollView
        ...
        <FinancialTemplate
          {...{
            ...
            cardList: [{ id: 'borrowings', title: 'BORROWING CARDS', list: financialCards }],
...
export default compose(withDynamicModuleLoader(getModule()), withTranslation())(DebtPositionScreen);
