...
import { addOtherAsset } from 'store/Asset/action';
...
const formatFormJson = require('assets/forms/layouts/asset-other-assets.json');
...
function OtherAssets({
  ...
  const data = useMemo(() => {
    ...
    if (editData) {
      const { _id, name, notes, datePurchased, purchasePrice, value, ownership, valueAsAt } =
        editData;
      if (!isNil(value)) {
  ...
  const callAPIAddData = useCallback(
    async (inputData, callback) => {
      ...
      dispatch(addOtherAsset(handledData)).then(results => {
        if (results) {
...
export default compose(withDynamicModuleLoader(getModule()))(OtherAssets);
