...
const i18nScope = 'screens.directPaymentJar';

function DirectPaymentJarScreen(props) {
  const { t } = props;
  ...
  const { monthlyInTotal, monthlyOutGroups, allLoaded } = useSelector(
    selectJarByKeyAndJar(
      AppConstants.listTypeByJars.Direct,
      AppConstants.listTypeJarByMoneyOutExpense.Direct,
    ),
  );
