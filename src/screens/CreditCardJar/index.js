...
const i18nScope = 'screens.creditCardJar';

function CreditCardJarScreen(props) {
  const { t } = props;
  ...
  const { monthlyInTotal, monthlyOutGroups, allLoaded } = useSelector(
    selectJarByKeyAndJar(
      AppConstants.listTypeByJars.Credit,
      AppConstants.listTypeJarByMoneyOutExpense.Credit,
    ),
  );
        ...
        <View style={[AppStyle.flex1, AppStyle.padX15, AppStyle.marginTop10]}>
          <View style={AppStyle.marginBottom10}>
            <MoneySMARTBreakdownCard
              ...
              data={[
                {
                  key: t(`${i18nScope}.estimatedTransferFromPrimaryAccount`),
