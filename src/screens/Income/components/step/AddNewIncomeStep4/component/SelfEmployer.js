...
const formatFormJson = require('assets/forms/layouts/create_self_employed.json');
function SelfEmployed({
  ...
  const dataForm = useMemo(() => {
    ...
    if (editIncome) {
      ...
      UtilLib.getFieldFromFormJSON(formLayout, 'nextPayDate').value = nextPayDate
        ? moment(nextPayDate).format('DD/MM/YYYY')
        : '';
      UtilLib.getFieldFromFormJSON(formLayout, 'business.nature').value = `${
        business?.nature || ''
      }`;
      if (flags.nextDates) {
        const nextPayDateField = UtilLib.getFieldFromFormJSON(formLayout, 'nextPayDate');
...
export default compose(withTranslation(), withDynamicModuleLoader(getModule()))(SelfEmployed);
