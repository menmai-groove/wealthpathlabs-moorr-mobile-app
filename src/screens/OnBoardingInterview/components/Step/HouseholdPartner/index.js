...
const i18nScope = 'screens.onBoardingInterview.householdPartner';

function HouseholdPartner(props) {
  useOnBackButtonPress(props.onBack);
  const { t, onPress: handleNextStep } = props;
  const styles = useThemedStyle(themedStyles, 'screens.onBoardingInterview.householdPartner');
  const fNameInput = useRef(null);
  const householdData = useSelector(selectHouseholdData);

  const { handleSubmit, control, errors, setValue } = useForm({
    defaultValues: { fName: '' },
    resolver: yupResolver(SchemaLib.onBoardingInterviewSchema.householdPartner),
    shouldFocusError: false,
  });

  useEffect(() => {
    setValue('fName', householdData?.partnerName || '');
  }, [householdData, setValue]);

  const onSubmit = data => {
    Keyboard.dismiss();
    fNameInput.current.blur();
    handleNextStep({ partnerName: data.fName });
  };
  ...
  return (
    <View style={styles.container}>
      <KeyboardAwareScrollView
        ...
        <Question emotion="smile" content={t(`${i18nScope}.question`)} />
        <View style={AppStyle.flex1} collapsable={false}>
          <Controller
            ...
            render={({ onChange, onBlur, value }) => (
              <InputField
                ref={fNameInput}
                placeholder={t(`${i18nScope}.partnerNamePlaceholder`)}
                required
                value={value}
                onBlur={onBlur}
...
export default compose(withTranslation())(HouseholdPartner);
